 ...
 */
package edu.uic.cs342.project4.client;
...
import edu.uic.cs342.project4.common.ChatData;
import edu.uic.cs342.project4.common.ChatUtils;
import edu.uic.cs342.project4.common.NodeData;
import edu.uic.cs342.project4.common.NodeInfo;

/**
 * ChatClient provides the GUI for the chat client
 * and also sets up communication with the server.
 */
public class ChatClient extends JFrame implements ActionListener {
	private static final long serialVersionUID = 1L;
	private JTextField MessageField, IPAddress, PortNumber;
	...
	public ChatClient() {
		...
		PortNumber = new JTextField(Integer.toString(ChatUtils.DEFAULT_PORT));
		...
		IPAddress.setFont(new Font(ChatUtils.FONT_NAME, Font.PLAIN,
				ChatUtils.FONT_MEDIUM));
		PortNumber.setFont(new Font(ChatUtils.FONT_NAME, Font.PLAIN,
				ChatUtils.FONT_MEDIUM));
		...
		clientListDisp.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		...
		listLabel.setFont(new Font(ChatUtils.FONT_NAME, Font.BOLD, ChatUtils.FONT_SMALL));
	...
	public void connection() throws IOException, ClassNotFoundException {
		if (!ConnectionFlag) {
			...
			try {
				ServerName = IPAddress.getText();
				ServerPort = Integer.parseInt(PortNumber.getText());				
				clientSocket = new Socket();   
				clientSocket.connect(new InetSocketAddress(ServerName, ServerPort), ChatUtils.TIMEOUT);
				...
				setTitle(ChatUtils.CLIENT_TITLE + " - " + ChatUtils.CLIENT_NAME + clientId);
			...
			try {
				...
				if (e.getSource() == SendToSelected
					...
					for (int i = selectedIndices.length - 1; i >= 0; i--) {
						recipients.add(listModel.getElementAt(selectedIndices[i]));
					}
				}
